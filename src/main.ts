...
app.use(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (files?.jobDescription?.length !== 1 || files?.cv?.length !== 1) {
      files?.jobDescription?.[0]?.path && (await unlink(files.jobDescription[0].path));
...
process.on('uncaughtException', async (error: Error, origin: NodeJS.UncaughtExceptionOrigin) => {
